...
const rawRouteTicket = async (variables) => {
    ...
    route.use(checkAuthentication(keys.public));
    
    route.post('/new', async (req, res) => {
        const { title, description, userID } = req.body;
        if (!title || !description || !userID) return res.status(400).json({ error: 'Missing required fields' });
        ...
        const ticket = await db.createTicket({ title, description, userID });
        res.json({UUID: ticket.rows[0], title, description, userID});
    });
    ...
    route.ws('/:UUID/ws', async (ws, req) => {
        ...
        internalEvents.on('dispatchMessage', (data) => {
